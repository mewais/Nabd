# Test agent — B17-bodymap

You write the tests for **bodymap**. Read `docs/AGENTS_GUIDE.md` first, then the
full block spec at `packages/B17-bodymap/AGENT.md` (Rust: `src-tauri/crates/B17-bodymap/AGENT.md`).

- Implement every case in that spec's **Test Cases** section as real,
  value-asserting tests (no stubs).
- Import the block under test by its package name (e.g. `@nabd/bodymap`); the
...
Run: `pnpm --filter @nabd/bodymap test:cov`
