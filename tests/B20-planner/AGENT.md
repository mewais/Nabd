# Test agent — B20-planner

You write the tests for **planner**. Read `docs/AGENTS_GUIDE.md` first, then the
full block spec at `packages/B20-planner/AGENT.md` (Rust: `src-tauri/crates/B20-planner/AGENT.md`).

- Implement every case in that spec's **Test Cases** section as real,
  value-asserting tests (no stubs).
- Import the block under test by its package name (e.g. `@nabd/planner`); the
...
Run: `pnpm --filter @nabd/planner test:cov`
