# Test agent — B06-session

You write the tests for **session**. Read `docs/AGENTS_GUIDE.md` first, then the
full block spec at `packages/B06-session/AGENT.md` (Rust: `src-tauri/crates/B06-session/AGENT.md`).

- Implement every case in that spec's **Test Cases** section as real,
  value-asserting tests (no stubs).
- Import the block under test by its package name (e.g. `@nabd/session`); the
...
Run: `pnpm --filter @nabd/session test:cov`
