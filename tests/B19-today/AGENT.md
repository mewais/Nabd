# Test agent — B19-today

You write the tests for **today**. Read `docs/AGENTS_GUIDE.md` first, then the
full block spec at `packages/B19-today/AGENT.md` (Rust: `src-tauri/crates/B19-today/AGENT.md`).

- Implement every case in that spec's **Test Cases** section as real,
  value-asserting tests (no stubs).
- Import the block under test by its package name (e.g. `@nabd/today`); the
...
Run: `pnpm --filter @nabd/today test:cov`
