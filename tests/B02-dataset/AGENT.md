# Test agent — B02-dataset

You write the tests for **dataset**. Read `docs/AGENTS_GUIDE.md` first, then the
full block spec at `packages/B02-dataset/AGENT.md` (Rust: `src-tauri/crates/B02-dataset/AGENT.md`).

- Implement every case in that spec's **Test Cases** section as real,
  value-asserting tests (no stubs).
- Import the block under test by its package name (e.g. `@nabd/dataset`); the
...
Run: `pnpm --filter @nabd/dataset test:cov`
