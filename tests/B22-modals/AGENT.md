# Test agent — B22-modals

You write the tests for **modals**. Read `docs/AGENTS_GUIDE.md` first, then the
full block spec at `packages/B22-modals/AGENT.md` (Rust: `src-tauri/crates/B22-modals/AGENT.md`).

- Implement every case in that spec's **Test Cases** section as real,
  value-asserting tests (no stubs).
- Import the block under test by its package name (e.g. `@nabd/modals`); the
...
Run: `pnpm --filter @nabd/modals test:cov`
